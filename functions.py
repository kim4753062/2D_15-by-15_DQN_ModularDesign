...
def _epsilon_Greedy_policy(args, Q_value: list, well_placement: list) -> tuple:
    ...
    if epsilon > exploration_or_exploitation: # Exploration, random.randint(a,b): Return random integer in [a, b] (a<=X<=b)
        # 2023-10-04: Modified to prevent duplication of well placement for Random well placement
        max_row, max_col = random.randint(1, args.gridnum_y), random.randint(1, args.gridnum_x)
        while Q_value_list_mask[max_row-1][max_col-1] == np.NINF:
            max_row, max_col = random.randint(1, args.gridnum_y), random.randint(1, args.gridnum_x)
...
############################# Policy Parameter Decline Trend #############################
# Define policy parameter (Tau for Boltzmann policy, Epsilon for Epsilon-greedy policy) decline trend
# Always can be customized by users
def policy_param_function(args, algorithm_iter_count: int) -> float:
    if algorithm_iter_count <= round(args.max_iteration * 0.3):
        policy_param = args.policy_param_start
    else:  # algorithm_iter_count >= round(args.max_iteration * 0.3) + 1:
        policy_param = args.policy_param_start + (args.policy_param_end - args.policy_param_start) * (algorithm_iter_count - round(args.max_iteration * 0.3)) / (args.max_iteration - round(args.max_iteration * 0.3))
    return policy_param
